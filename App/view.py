...
def printRequirement2(requirement_list, num_purchased_artworks, catalog):
    total_num_artworks = lt.size(requirement_list)
    first_artists = lt.subList(requirement_list, 1, 3)
    last_artists = lt.subList(requirement_list, total_num_artworks - 2, 3)
    print('Existen', total_num_artworks, 'obras de arte adquiridas en el rango de fechas indicado')
    ...
    for artwork in lt.iterator(last_artists):
        authors = ''
        for author_Id in controller.GetConstituentIDList(artwork['ConstituentID']):
            author_name = me.getValue(mp.get(catalog['artists'], author_Id))['info']['DisplayName']
            authors += ' ,' + author_name
        authors = authors[2:]
    ...
    for artwork in lt.iterator(first_artists):
        authors = ''
        for author_Id in controller.GetConstituentIDList(artwork['ConstituentID']):
            author_name = me.getValue(mp.get(catalog['artists'], author_Id))['info']['DisplayName']
            authors += ' ,' + author_name
...
def printMenu():
    ...
    print('3- REQ. 2: listar cronológicamente las adquisiciones (Grupal)')
    print('')
    print('0- Salir')
